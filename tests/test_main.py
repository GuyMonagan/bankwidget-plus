...
def fake_inputs(monkeypatch):
    # Эмулируем: выбор 1 → дата → выход
    inputs = iter(["1", "2025-01-01 00:00:00", "0"])
    monkeypatch.setattr("builtins.input", lambda _: next(inputs))


@patch("bankwidget_plus.main.load_data")
def test_main_with_fake_data(mock_load_data, fake_inputs, capsys):
    # Подменяем load_data, чтобы он вернул фейковый DataFrame
...
@patch("bankwidget_plus.main.load_data")
def test_main_weekday_report(mock_load_data, monkeypatch, capsys):
    mock_load_data.return_value = pd.DataFrame([
        {"Дата операции": "2025-01-08", "Сумма платежа": 100.0},
        {"Дата операции": "2025-01-09", "Сумма платежа": 200.0},
    ])
    inputs = iter(["2", "2025-01-01", "0"])
    monkeypatch.setattr("builtins.input", lambda _: next(inputs))
...
@patch("bankwidget_plus.main.load_data")
def test_main_invalid_choice(mock_load_data, monkeypatch, capsys):
    mock_load_data.return_value = pd.DataFrame()
    inputs = iter(["42", "0"])
    monkeypatch.setattr("builtins.input", lambda _: next(inputs))
...
@patch("bankwidget_plus.main.load_data")
def test_main_exit_immediately(mock_load_data, monkeypatch, capsys):
    mock_load_data.return_value = pd.DataFrame()
